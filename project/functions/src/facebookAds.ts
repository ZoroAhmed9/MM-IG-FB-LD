...
export const createAdSetAndAd = functions.https.onCall(
  async (data: CreateAdRequest, context: functions.https.CallableContext): Promise<FacebookAdResponse> => {
    ...
    try {
      ...
      if (!context.auth) {
        ...
        throw new functions.https.HttpsError(
          'unauthenticated',
          'User must be authenticated to create ads'
        );
      }
...
async function createAd(
  ...
  const response = await fetch(`https://graph.facebook.com/v19.0/act_${adAccountId}/ads`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
